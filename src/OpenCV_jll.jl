...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import OpenCV_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("OpenCV_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "OpenCV_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"OpenCV", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "OpenCV_jll", "src", "OpenCV_jll.jl"))
        @info("OpenCV_ll dev'ed out to /home/terasaki/.julia/dev/OpenCV_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("33b9d88c-85f9-5d73-bd91-4e2b95a9aa0b"))
platforms = [Pkg.Artifacts.unpack_platform(e, "OpenCV", artifacts_toml) for e in artifacts["OpenCV"]]
...
if best_platform === nothing
    @debug("Unable to load OpenCV; unsupported platform $(triplet(platform_key_abi()))")
else
